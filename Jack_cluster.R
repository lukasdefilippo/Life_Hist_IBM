...
rec_func <- function(S=10000, alpha=60, beta=0.00017, sigma_R=0.0, s_juv=1, form='B-H'){
...
rec_vec <- seq(0, 1.0, 0.1)
slope_vec <- seq(0, 0.6, 0.05)
#growth_vec <- c(0.1, 0.2, 0.3)
#int_vec <- c(0.0001, 0.01, 0.05)
growth_vec <- c(0.2)
int_vec <- c(0.01)

#List for storing the total jack prevalence by brood year
jack_mat_a <- replicate(length(growth_vec), list(replicate(length(int_vec), list(matrix(ncol=length(slope_vec), nrow=length(rec_vec))))))
#List for storing the prevalence of jacks by return year
jack_mat_return_a <- replicate(length(growth_vec), list(replicate(length(int_vec), list(matrix(ncol=length(slope_vec), nrow=length(rec_vec))))))
#List for storing the prevalence of jacks by brood year among males only
jack_mat_male_a <- replicate(length(growth_vec), list(replicate(length(int_vec), list(matrix(ncol=length(slope_vec), nrow=length(rec_vec))))))
#List for storing the prevalence of jacks by return year among males only
jack_mat_return_male_a <- replicate(length(growth_vec), list(replicate(length(int_vec), list(matrix(ncol=length(slope_vec), nrow=length(rec_vec))))))
  ...
  for(r in 1:length(rec_vec)){
    for(g in 1:length(growth_vec)){
      for(k in 1:length(int_vec)){
        burnin <- 1   # Deterministic burnin
        n_year <- 1000   # Total length of simulation
        n_age <- 5      # Maximum number of age classes
        n_sim <- 2      # Number of stochastic simulations to run
        ...
        for(l in 1:n_sim){
          ...
          for (y in 1:n_year){
            #year 1
            if(y==1){
              #initiate rectuits based on the asymptotic recruitment of the assumed S-R function
              recruits <- S_R_alpha/S_R_beta 
              #Calculate the sex ratio of the recruits as 0.5
              ...
              for(a in 1:n_age){
                ...
                if(a==1){
                  ...
                  # 2) L_infinity drawn from population wide-distribution --> column 2
                  # 3) Growth coeffient (deterministic function of Linfinity) --> column 3
                  # 4) Length (calculated via stochastic von-bertalanfy) --> column 4
                  
                  #loop through sex 
                  for(s in 1:n_sex){
                    for(j in 1:n_sire){
                      #if individuals are male and were sired by a jack, they inherit a PMRN intercept for jacks (randomly assigned)
                      if(s==1){
                ...
                if(a > 1 & a < 3){
                  ...
                  for(s in 1:n_sex){
                    for(j in 1:n_sire){
                      ...
                      imm_list[[y]][[a]][[s]][[j]][[l]] <- adult_surv(input=imm_list[[y]][[a-1]][[s]][[j]][[l]], s_adult=1) #survival is 100% for early life (all juvenile morality is integrated into the S-R function)
                  ...
                  for(s in 1:n_sex){
                    for(j in 1:n_sire){
                      ...
                      if(is.null(dim(imm_list[[y]][[a-1]][[s]][[j]][[l]]))==FALSE){
                        ...
                        if(a ==3 & s==1){
                          mature <- rbinom(n=nrow(imm_list[[y]][[a]][[s]][[j]][[l]]), size=1, prob=0.01) #low probability of maturation at ocean age-3 for females
                        }else{
            ...
            if(y > 1){ #for years greater than one
                ...
                for (s in 1:n_sex){
                  for (j in 1:n_sire){
                    if(is.null(dim(mat_list[[y-1]][[a]][[s]][[j]][[l]]))==FALSE){
                      total_return <- total_return + nrow(mat_list[[y-1]][[a]][[s]][[j]][[l]]) #looping through all ages, sex, and sire types, add the fish that matured in the previous year to the total return
                    }
                  }
                }
              }
              ...
              jacks <- rbind(mat_list[[y-1]][[3]][[2]][[1]][[l]], mat_list[[y-1]][[3]][[2]][[2]][[l]])
              ...
              hook <- matrix(rbind(mat_list[[y-1]][[4]][[2]][[1]][[l]], mat_list[[y-1]][[4]][[2]][[2]][[l]]), ncol=4)
              hook_age_comp[1] <- nrow(matrix(rbind(mat_list[[y-1]][[4]][[2]][[1]][[l]], mat_list[[y-1]][[4]][[2]][[2]][[l]]), ncol=4))
              
              if(n_age > 4){ #if the total number of ages exceeds four
                #loop over remaining ages
                for (a in 5:n_age){
                  hook <- matrix(rbind(hook, mat_list[[y-1]][[a]][[2]][[1]][[l]], mat_list[[y-1]][[a]][[2]][[2]][[l]]), ncol=4) #Grab age 5 males of both sire types
                  hook_age_comp[a-3] <- nrow(mat_list[[y-1]][[a]][[2]][[1]][[l]]) + nrow(mat_list[[y-1]][[a]][[2]][[2]][[l]]) #Grab the number of age 5 males of both sire types
                }
              }
              ...
              hook_age_comp <- hook_age_comp/nrow(hook) #divide the numbers of each hooknose age class by the total number of hooknose males altogether
              
              #Create an object for storing females --> initiate at age 3 (should be rare but allowed), merge across sire types
              females <- matrix(rbind(mat_list[[y-1]][[3]][[1]][[1]][[l]], mat_list[[y-1]][[3]][[1]][[2]][[l]]), ncol=4)
              ...
              for (a in 4:n_age){
                ...
                females <- matrix(rbind(females, cbind(mat_list[[y-1]][[a]][[1]][[1]][[l]], rep(a, nrow(mat_list[[y-1]][[a]][[1]][[1]][[l]]))), cbind(mat_list[[y-1]][[a]][[1]][[2]][[l]], rep(a, nrow(mat_list[[y-1]][[a]][[1]][[2]][[l]])))), ncol=5)
              }
              ...
              #sampling with replacement because males can fertilize more than one female's offspring
              ...
              #calculate weighted probability of survival for hooknose males
              hook_surv <- vector(length=length(hook_age_comp))
              for(a in 1:length(hook_age_comp)){
                hook_surv[a] <- hook_age_comp[a]*(s_adult[a+1]^a)
              }
        ...
        if(plots==TRUE){
          pdf(file=paste('sigma_R=',sigma_R_g,'sigma_g=',sd.g_g ,'slope=',slope_vec[w],'n_age=',n_age,'mate_eq=',mate_eq,'.pdf'))
          ...
          plot(apply(jack_p_run_total, 1, mean, na.rm=TRUE), type='l', col='black', ylim=c(0, max(apply(jack_p_run_total, 1, cred)[2,])), lwd=1.5, xlab='Year', ylab=
                 'Jack proportion', main='Jack proportion (return year)',cex.main=0.85, cex=0.85)
          mtext(side=1, 'Year', cex=0.85, line=2)
              ...
              for(a in 1:n_age){
                for(s in 1:n_sex){
                  for(j in 1:n_sire){
                    #conditional to avoid errors from empty lists
                    if(is.null(dim(mat_list[[y]][[a]][[s]][[j]][[l]]))==FALSE){
                      total_spawners[y,l] <- total_spawners[y,l] + nrow(mat_list[[y]][[a]][[s]][[j]][[l]]) 
                    }
                  }
                }
              }
          ...
          plot(apply(total_spawners, 1, mean, na.rm=TRUE), type='l', col='black', ylim=c(0, max(apply(total_recruits[-1,], 1, cred)[2,])), lwd=1.5, xlab='Year', ylab=
          ...
          polygon(x=xx, y=yy, col=rgb(100,1,1, max=250, alpha=90), border=FALSE)
          legend(x=100, y=1.05*max(apply(total_recruits[-1,], 1, cred)[2,]), legend=c('spawners', 'recruits'), cex=0.65, col=c('black', 'red'), lty=1, bty='n')
          ...
          plot(apply(jack_p_brood_total, 1, mean, na.rm=TRUE), type='l', col='black', ylim=c(0, max(apply(jack_p_brood_total, 1, cred)[2,])), lwd=1.5, xlab='Year', ylab=
                 'Jack proportion', main='Jack proportion (brood year)',cex.main=0.85, cex=0.85)
          mtext(side=1, 'Year', cex=0.85, line=2)
          mtext(side=2, 'Proportion', cex=0.85, line=2)
          ...
          plot(apply(jack_p_brood_total, 1, mean, na.rm=TRUE), type='l', col='red', ylim=c(0, max(apply(jack_p_run_total, 1, cred)[2,])), lwd=1.5, xlab='Year', ylab=
                 'Jack prop', main='Jack prop (brood and return)', cex.main=0.85, cex=0.85)
          points(apply(jack_p_run_total, 1, mean, na.rm=TRUE), type='l', col='blue')
          legend(x=100, y=max(apply(jack_p_run_total, 1, cred)[2,]), legend=c('jacks in spawners', 'jacks in return'), cex=0.65, col=c('red', 'blue'), lty=1, bty='n')
          mtext(side=1, 'Year', cex=0.85, line=2)
          mtext(side=2, 'Proportion', cex=0.85, line=2)
          ...
          for(a in 4:n_age){
            points(apply(age_comp_brood_female[,,a], 1, mean, na.rm=TRUE), type='l', col=rgb(col2rgb(col=col.vec[a])[1], col2rgb(col=col.vec[a])[2], col2rgb(col=col.vec[a])[3], max=255, alpha=255), ylim=c(0,1), lwd=1.5, xlab='Year', ylab=
                     'Proportion-at-age', main='female age (brood year)',cex.main=0.5, cex=0.5)
            xx <- c((1:n_brood), rev(1:n_brood))
          ...
          for(a in 4:n_age){
            points(apply(age_comp_brood_male[,,a], 1, mean), type='l', col=rgb(col2rgb(col=col.vec[a])[1], col2rgb(col=col.vec[a])[2], col2rgb(col=col.vec[a])[3], max=255, alpha=255), ylim=c(0,1), lwd=1.5, xlab='Year', ylab=
                     'Proportion-at-age', main='male age (brood year)',cex.main=0.5, cex=0.5)
            xx <- c((1:n_brood), rev(1:n_brood))
          ...
          #age comp by return year (escapement)
          n_esc <- n_year
            ...
            for(l in 1:n_sim){
              for(a in 1:n_age){
                for(s in 1:n_sex){
                  if(s==2){
                    if(is.null(dim(mat_list[[y]][[a]][[s]][[j]][[l]]))==FALSE){
                      age_comp_esc_male[y,l,a] <- (nrow(mat_list[[y]][[a]][[s]][[1]][[l]]) + nrow(mat_list[[y]][[a]][[s]][[2]][[l]]))/total_spawner_male[y,l]
                    }
                  }
                  if(s==1){
                    if(is.null(dim(mat_list[[y]][[a]][[s]][[j]][[l]]))==FALSE){
                      age_comp_esc_female[y,l,a] <- (nrow(mat_list[[y]][[a]][[s]][[1]][[l]]) + nrow(mat_list[[y]][[a]][[s]][[2]][[l]]))/total_spawner_female[y,l]
                    }
                  }
                }
              }
          ...
          # #plot time series (females)
          # col.vec <- brewer.pal(n=(n_age), name='Blues')
          # plot(apply(age_comp_esc_female[,,3], 1, mean, na.rm=TRUE), type='l', col=rgb(col2rgb(col=col.vec[3])[1], col2rgb(col=col.vec[3])[2], col2rgb(col=col.vec[3])[3], max=255, alpha=255), ylim=c(0,1), lwd=1.5, xlab='Year', ylab=
          #        'Proportion-at-age', main='female age in escapement', cex.main=0.85, cex=0.85)
          # mtext(side=1, 'Year')
          # mtext(side=2, 'Proportion')
          # xx <- c((1:n_esc), rev(1:n_esc))
          # yy <- c(apply(age_comp_esc_female[,,3], 1, cred)[1,], rev(apply(age_comp_esc_female[,,3], 1, cred)[2,]))
          # polygon(x=xx, y=yy, col=rgb(col2rgb(col=col.vec[3])[1], col2rgb(col=col.vec[3])[2], col2rgb(col=col.vec[3])[3], max=255, alpha=200), border=FALSE)
          # for(a in 4:n_age){
          #   points(apply(age_comp_esc_female[,,a], 1, mean, na.rm=TRUE), type='l', col=rgb(col2rgb(col=col.vec[a])[1], col2rgb(col=col.vec[a])[2], col2rgb(col=col.vec[a])[3], max=255, alpha=255), ylim=c(0,1), lwd=1.5)
          #   xx <- c((1:n_esc), rev(1:n_esc))
          ...
          # #plot time series (males)
          # plot(apply(age_comp_esc_male[,,3], 1, mean, na.rm=TRUE), type='l', col=rgb(col2rgb(col=col.vec[3])[1], col2rgb(col=col.vec[3])[2], col2rgb(col=col.vec[3])[3], max=255, alpha=255), ylim=c(0,1), lwd=1.5, xlab='Year', ylab=
          #        'Proportion-at-age', main='male age in escapement', cex.main=0.85, cex=0.85)
          # mtext(side=1, 'Year')
          # mtext(side=2, 'Proportion')
          # xx <- c((1:n_esc), rev(1:n_esc))
          ...
          # polygon(x=xx, y=yy, col=rgb(col2rgb(col=col.vec[3])[1], col2rgb(col=col.vec[3])[2], col2rgb(col=col.vec[3])[3], max=255, alpha=200), border=FALSE)
          # for(a in 4:n_age){
          #   points(apply(age_comp_esc_male[,,a], 1, mean, na.rm=TRUE), type='l', col=rgb(col2rgb(col=col.vec[a])[1], col2rgb(col=col.vec[a])[2], col2rgb(col=col.vec[a])[3], max=255, alpha=255), ylim=c(0,1), lwd=1.5)
          #   xx <- c((1:n_esc), rev(1:n_esc))
          ...
          #males
          plot(apply(y_0_recruit_male_mu[,,1], 1, median, na.rm=TRUE), ylim=c(100,1800), type='l', col='blue', ylab='PMRN intercept', xlab='year', main='male PMRN intercept', cex.main=0.85, cex=0.85)
          ...
          #females
          plot(apply(y_0_recruit_female_mu[,,1], 1, median, na.rm=TRUE), ylim=c(100,1800), type='l', col='blue', ylab='PMRN intercept', xlab='year', main='female PMRN intercept', cex.main=0.85, cex=0.85)
          ...
          #plot the median of the median trait value among simulations, as well as the median upper and lower bounds among simulations
          #males
          plot(apply(L_inf_recruit_male_mu[,,1], 1, median, na.rm=TRUE), ylim=c(200,1000), type='l', col='blue', ylab='Asymptotic length', xlab='year', main='male asymptotic length', cex.main=0.85, cex=0.85)
          ...
          #females
          plot(apply(L_inf_recruit_female_mu[,,1], 1, median, na.rm=TRUE), ylim=c(200,1000), type='l', col='blue', ylab='Asymptotic length', xlab='year', main='female asymptotic length', cex.main=0.85, cex=0.85)
          ...
          #plot the median of the median trait value among simulations, as well as the median upper and lower bounds among simulations
          #males
          plot(apply(K_brody_recruit_male_mu[,,1], 1, median, na.rm=TRUE), ylim=c(0,1), type='l', col='blue', ylab='Growth rate', xlab='year', main='male growth rate', cex.main=0.85, cex=0.85)
          ...
          #females
          plot(apply(K_brody_recruit_female_mu[,,1], 1, median, na.rm=TRUE), ylim=c(0,1), type='l', col='blue', ylab='Growth rate', xlab='year', main='female growth rate', cex.main=0.85, cex=0.85)
        ...
        jack_mat_a[[g]][[k]][r,w]   <- median(apply(jack_p_brood_total[(n_year-250):(n_year-n_age-1),],2, mean, na.rm=TRUE),na.rm=TRUE)
        jack_mat_return_a[[g]][[k]][r,w]   <- median(apply(jack_p_run_total[(n_year-250):(n_year-n_age-1),],2, mean, na.rm=TRUE),na.rm=TRUE)
        jack_mat_male_a[[g]][[k]][r,w]   <- median(apply(jack_p_brood_total_male[(n_year-250):(n_year-n_age-1),],2, mean, na.rm=TRUE),na.rm=TRUE)
        jack_mat_return_male_a[[g]][[k]][r,w]   <- median(apply(jack_p_run_total_male[(n_year-250):(n_year-n_age-1),],2, mean, na.rm=TRUE),na.rm=TRUE)
      }
    }
  }
}

saveRDS(jack_mat_a, file="jack_mat_a.RDS")
saveRDS(jack_mat_return_a, file="jack_mat_return_a.RDS")
saveRDS(jack_mat_male_a, file="jack_mat_male_a.RDS")
saveRDS(jack_mat_return_male_a, file="jack_mat_return_male_a.RDS")
